...
namespace NsqSharp.Tests.Utils
{
    ...
    public class TimerTest
    {
        ...
        {
            var timer = new NsqSharp.Utils.Timer(TimeSpan.FromSeconds(1));
            bool ticked = false;
            new SelectCase()
        ...
        {
            var timer = new NsqSharp.Utils.Timer(TimeSpan.FromSeconds(3));
            bool ticked = false;
            new SelectCase()
                .CaseReceive(timer.C, _ => ticked = true)
                ...
                GoFunc.Run(() =>
                           {
                               try
                               {
                                   ...
                                   var timer = new NsqSharp.Utils.Timer(TimeSpan.FromMilliseconds(time));
